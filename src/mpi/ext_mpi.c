...
static int allgatherv_init_general(const void *sendbuf, int sendcount,
  ...
  *handle = EXT_MPI_Allgatherv_init_native(
      ...
      num_ports, groups,
      my_cores_per_node_row * my_cores_per_node_column, alt, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, ext_mpi_num_sockets_per_node, 0);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
static int reduce_scatter_init_general(
  ...
  *handle = EXT_MPI_Reduce_scatter_init_native(
      ...
      copyin_method_, copyin_factors, alt, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, num_sockets_per_node, 0);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
static int scatterv_init_general(const void *sendbuf, const int *sendcounts, const int *displs,
  ...
  *handle = EXT_MPI_Scatterv_init_native(
      ...
      root, comm_row, my_cores_per_node_row, comm_column,
      my_cores_per_node_column, num_ports, groups,
      my_cores_per_node_row * my_cores_per_node_column, copyin_method, alt, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, ext_mpi_num_sockets_per_node, 0);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
static int allreduce_init_general(const void *sendbuf, void *recvbuf, int count,
  ...
  *handle = EXT_MPI_Allreduce_init_native(
      ...
      ext_mpi_bit_identical, !ext_mpi_bit_reproducible, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, num_sockets_per_node, 0);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
static int reduce_init_general(const void *sendbuf, void *recvbuf, int count,
  ...
  *handle = EXT_MPI_Reduce_init_native(
      ...
      0, !ext_mpi_bit_reproducible, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, num_sockets_per_node, 0);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
static int bcast_init_general(void *buffer, int count, MPI_Datatype datatype,
  ...
  *handle = EXT_MPI_Bcast_init_native(
      ...
      comm_column, my_cores_per_node_column, num_ports, groups,
      my_cores_per_node_row * my_cores_per_node_column, copyin_method_, copyin_factors, alt, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, num_sockets_per_node, 0);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
int EXT_MPI_Init_blocking() {
  int comm_size_row, comm_rank_row, i, comm_size_column, message_size, type_size, *num_ports = NULL, *groups = NULL, group_size, copyin_method_, *copyin_factors = NULL, num_sockets_per_node, alt;
  double d1;
  struct cost_list *p1, *p2;
  int count = 1;
  ...
  if (fixed_factors_ports == NULL && comm_size_row == my_cores_per_node * ext_mpi_num_sockets_per_node && ext_mpi_num_sockets_per_node == 2) {
    ...
    if (comm_size_row / my_cores_per_node > 1) {
      if (my_cores_per_node == 1) {
        ...
        if (ext_mpi_cost_simulation(count, type_size, comm_size_row, my_cores_per_node,
                                           1, 1,
                                           comm_size_row, comm_rank_row, 0, ext_mpi_bit_identical, ext_mpi_num_sockets_per_node) < 0)
          goto error;
      }
  ...
  if (copyin_method < 0) {
    ...
    if ((count * type_size <= CACHE_LINE_SIZE - OFFSET_FAST) && ext_mpi_blocking) {
      ...
      copyin_method =
          (count * type_size > ext_mpi_node_size_threshold[my_cores_per_node - 1]);
    }
  }
  alt = 0;
  if (alternating >= 1) {
...
alt = 0;
  ...
  if (EXT_MPI_Init_blocking_native(1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD, my_cores_per_node, num_ports, groups, copyin_method_, copyin_factors, 0, 0, (group_size==comm_size_row/my_cores_per_node) && !not_recursive, ext_mpi_blocking, num_sockets_per_node) < 0)
    goto error;
  free(groups);
  free(num_ports);
  free(copyin_factors);
