...
static int allreduce_init_general(const void *sendbuf, void *recvbuf, int count,
  ...
  *handle = EXT_MPI_Allreduce_init_native(
      ...
      ext_mpi_bit_identical, !ext_mpi_bit_reproducible, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, num_sockets_per_node, 0, NULL);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
...
static int reduce_init_general(const void *sendbuf, void *recvbuf, int count,
  ...
  *handle = EXT_MPI_Reduce_init_native(
      ...
      0, !ext_mpi_bit_reproducible, (group_size==comm_size_row/my_cores_per_node_row) && !not_recursive, ext_mpi_blocking, num_sockets_per_node, 0, NULL);
  if (*handle < 0)
    goto error;
  free(groups);
  free(num_ports);
