...
int EXT_MPI_Allgatherv_init_general(const void *sendbuf, int sendcount,
  ...
  MPI_Allgatherv(sendbuf_h, sendcount, sendtype, recvbuf_ref, recvcounts, displs,
                 recvtype, comm_row);
  allgatherv_init_general(sendbuf_h, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm_row, my_cores_per_node_row, comm_column, my_cores_per_node_column, handle);
...
int EXT_MPI_Allreduce_init_general(const void *sendbuf, void *recvbuf, int count,
  ...
  if (gpu_is_device_pointer(recvbuf)) {
    ...
    if (!recvbuf_ref)
      goto error;
    if (sendbuf != MPI_IN_PLACE){
      gpu_malloc(&sendbuf_h, count * type_size);
      if (!sendbuf_h)
        goto error;
      ...
      gpu_memcpy_hd(sendbuf_h, sendbuf_hh, count * type_size);
      gpu_memcpy_hd(recvbuf, recvbuf_hh, count * type_size);
    }else{
      ...
      gpu_memcpy_hd(recvbuf, recvbuf_hh, count * type_size);
      gpu_memcpy_hd(recvbuf_ref, recvbuf_hh, count * type_size);
    }
    if (type_size == sizeof(long int)){
    ...
    if (EXT_MPI_Done_native(*handle) < 0)
      goto error;
    gpu_memcpy_dh(recvbuf_hh, recvbuf, count * type_size);
    gpu_memcpy_dh(recvbuf_ref_hh, recvbuf_ref, count * type_size);
    j = 0;
    for (i = 0; i < (count * type_size) / (int)sizeof(long int); i++) {
