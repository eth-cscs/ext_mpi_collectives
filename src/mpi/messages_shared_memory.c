...
int ext_mpi_messages_shared_memory(char *buffer_in, char *buffer_out, MPI_Comm comm_row,
  ...
  int nbuffer_out = 0, integer1, ascii, node_rank;
  ...
  while ((integer1 = ext_mpi_read_line(buffer_in, line, ascii)) > 0) {
    ...
    if (estring1 == ewaitall) {
      ...
      while (data_irecv_isend_list_temp) {
        MPI_Wait(&data_irecv_isend_list_temp->request, MPI_STATUS_IGNORE);
        data_memcpy_reduce.buffer_type1 = data_irecv_isend_list_temp->data_irecv_isend_ref.buffer_type;
        data_memcpy_reduce.buffer_number1 = data_irecv_isend_list_temp->data_irecv_isend_ref.buffer_number;
        data_memcpy_reduce.is_offset1 = data_irecv_isend_list_temp->data_irecv_isend_ref.is_offset;
        data_memcpy_reduce.offset_number1 = data_irecv_isend_list_temp->data_irecv_isend_ref.offset_number;
        data_memcpy_reduce.offset1= data_irecv_isend_list_temp->data_irecv_isend_ref.offset;
        data_memcpy_reduce.buffer_type2 = data_irecv_isend_list_temp->data_irecv_isend.buffer_type;
        data_memcpy_reduce.buffer_number2 = data_irecv_isend_list_temp->data_irecv_isend.buffer_number;
        data_memcpy_reduce.is_offset2 = data_irecv_isend_list_temp->data_irecv_isend.is_offset;
        data_memcpy_reduce.offset_number2 = data_irecv_isend_list_temp->data_irecv_isend.offset_number;
        data_memcpy_reduce.offset2 = data_irecv_isend_list_temp->data_irecv_isend.offset;
        data_memcpy_reduce.size = data_irecv_isend_list_temp->data_irecv_isend.size;
      }
      if (node_rank == 0) {
    ...
    if ((estring1 == eisend) || (estring1 == eirecv)) {
      ...
      if (estring1 == eisend) {
        ...
        MPI_Isend(&data_irecv_isend_list_send->data_irecv_isend, sizeof(data_irecv_isend_list_send->data_irecv_isend), MPI_CHAR, data_irecv_isend.partner, 0, MPI_COMM_WORLD, &data_irecv_isend_list_send->request);
      } else {
        data_irecv_isend_list_temp = (struct line_irecv_isend_list*)malloc(sizeof(*data_irecv_isend_list_temp));
        ...
        MPI_Irecv(&data_irecv_isend_list_recv->data_irecv_isend, sizeof(data_irecv_isend_list_recv->data_irecv_isend), MPI_CHAR, data_irecv_isend.partner, 0, MPI_COMM_WORLD, &data_irecv_isend_list_recv->request);
      }
    }
