...
int generate_backward_interpreter(char *buffer_in, char *buffer_out, MPI_Comm comm_row){
  ...
  for (i=0; i<size_level1[size_level0-1]; i++){
    for (j=0; j<data[size_level0-1][i].to_max; j++){
      if ((data[size_level0-1][i].to[j]==-1)&&(parameters->node==parameters->root/parameters->node_row_size)){
    ...
    for (j=size_level1[i-1]-1; j>=0; j--){
      for (k=0; k<data[i-1][j].to_max; k++){
        if (data[i-1][j].to[k]!=parameters->node){
          MPI_Irecv(&recv_values[j][data[i-1][j].to[k]], 1, MPI_INT, data[i-1][j].to[k]*parameters->node_row_size+parameters->node_rank%parameters->node_row_size, 0, comm_row, &request[l++]);
        }
      }
    }
    for (j=size_level1[i]-1; j>=0; j--){
      for (k=0; k<data[i][j].from_max; k++){
        if (data[i][j].from_node[k]==parameters->node){
          ...
          MPI_Isend(&values[i][j], 1, MPI_INT, data[i][j].from_node[k]*parameters->node_row_size+parameters->node_rank%parameters->node_row_size, 0, comm_row, &request[l++]);
        }
      }
    }
    for (j=size_level1[i-1]-1; j>=0; j--){
