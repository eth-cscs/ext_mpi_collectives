...
static double execution_time(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
  ...
  handle = EXT_MPI_Allreduce_init_native(sendbuf, recvbuf, count, datatype, op,
    ...
    groups_, copyin_method, copyin_factors, 1, 0, 0, 0, 0, num_sockets_per_node, 0, NULL, NULL, NULL, NULL);
  iterations = 10;
  flag = 1;
  while (flag) {
    iterations *= 2;
...
static double allreduce_measurement(const void *sendbuf, void *recvbuf, int count,
  ...
  *copyin_method = -1;
  ...
  if (lmsize <= CACHE_LINE_SIZE - OFFSET_FAST && !on_gpu && mpi_size_row == my_cores_per_node_row) {
    *copyin_method = 0;
    factors_max = 0;
