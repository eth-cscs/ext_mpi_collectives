    ...
    *copyin_method = 0;
    ...
    if (i != my_cores_per_node_row) {
      *copyin_method = 1;
      ...
      copyin_factors_min[factors_max] = copyin_factors[factors_max] = 0;
      time_min = execution_time(sendbuf, recvbuf, count, datatype, op, comm_row_, my_cores_per_node_row, comm_column_, my_cores_per_node_column, *copyin_method, copyin_factors);
    }
    ...
    for (i = 0; i <= factors_max; i++){
      ...
      copyin_factors[k] = 0;
      time = execution_time(sendbuf, recvbuf, count, datatype, op, comm_row_, my_cores_per_node_row, comm_column_, my_cores_per_node_column, 0, copyin_factors);
      k = time < time_min;
