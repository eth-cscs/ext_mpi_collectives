...
int alltoall_get_text(char *buffer_in, char *buffer_out){
  ...
  int i, gbstep, nstep, shift, nbuffer_out=0, *source_array, from, to, flag, integer1;
  ...
  for (gbstep=1,nstep=0; gbstep<num_nodes*(num_ports+1); gbstep*=(num_ports+1),nstep++){
    for (i=0; i<num_nodes; i++){
      if (nstep>0){
        shift=((i/(gbstep/(num_ports+1)))%(num_ports+1))*(gbstep/(num_ports+1));
      ...
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " STAGE %d FRAC %d SOURCE %d TO %d FROM %d|-1\n", nstep, (i+task)%num_nodes, source_array[i], to, from);
