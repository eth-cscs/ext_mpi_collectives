  ...
  *stages = (struct allreduce_data_element *)malloc((nsteps+1)*sizeof(struct allreduce_data_element));
  ...
  if (!stages_in){
    for (step=0; step<=nsteps; step++){
      ...
      (*stages)[step].from_value = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*max_port_from*num_processors);
      if (!(*stages)[step].from_value) goto error;
      (*stages)[step].from_line = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*max_port_from*num_processors);
      ...
      (*stages)[step].to_value = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*(max_port_to+1));
...
static int allreduce_init(struct allreduce_data_element **stages, int num_processors, int *num_ports, int task, int allreduce){
...
//        (*stages)[step+1].from_line[chunk*(port+1)+i] = chunk*(port+1)+i;
  ...
  for (k=0; k<=frac_max; k++){
    ...
    if (m>=0){
      (*stages)[step].to_value[(*stages)[step].num_to[m]*(*stages)[step].max_lines+m] = -1;
      (*stages)[step].num_to[m]++;
    }
    for (j=l+1; j<num_processors; j++){
      for (i=0; i<(*stages)[step].max_lines; i++){
        if (((*stages)[step].frac[i]==k)&&((*stages)[step].source[i]==j)){
          (*stages)[step].from_value[(*stages)[step].max_lines*(*stages)[step].num_from[m]+m] = task;
          (*stages)[step].from_line[(*stages)[step].max_lines*(*stages)[step].num_from[m]+m] = i;
