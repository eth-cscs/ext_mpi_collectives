...
static int read_int_series(char *string_in, int **integers_out){
  ...
  while((*string_in_new!='\0')&&i){
    while((*string_in_new!=' ')&&(*string_in_new!='\0')&&i){
      if (((*string_in_new>='0')&&(*string_in_new<='9'))||(*string_in_new=='-')){
        string_in_new++;
      }else{
        i=0;
      }
  ...
  *integers_out = (int*) malloc((i_max+1)*sizeof(int));
  if (!*integers_out) return ERROR_MALLOC;
  string_in_new=string_in;
  for (i=0; i<i_max; i++){
...
int read_parameters(char *buffer_in, struct parameters_block **parameters){
  char string1[100], string2[100], *buffer_in_new, *buffer_in_copy, *buffer_in_pcopy=NULL;
  int nbuffer_in=0, integer1, flag;
  *parameters=(struct parameters_block*)malloc(sizeof(struct parameters_block));
  if (!*parameters) return ERROR_MALLOC;
  ...
  if (!(*parameters)->ascii_in){
    ...
    if ((*parameters)->counts_max){
      (*parameters)->counts=(int*)malloc(sizeof(int)*((*parameters)->counts_max+1));
      if (!(*parameters)->counts) goto error;
      memcpy((*parameters)->counts, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->counts_max);
      (*parameters)->counts[(*parameters)->counts_max]=0;
      nbuffer_in+=sizeof(int)*(*parameters)->counts_max;
    }
    if ((*parameters)->num_ports_max){
      (*parameters)->num_ports=(int*)malloc(sizeof(int)*((*parameters)->num_ports_max+1));
      if (!(*parameters)->num_ports) goto error;
      memcpy((*parameters)->num_ports, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->num_ports_max);
      (*parameters)->num_ports[(*parameters)->num_ports_max]=0;
      nbuffer_in+=sizeof(int)*(*parameters)->num_ports_max;
    }
    if ((*parameters)->groups_max){
      (*parameters)->groups=(int*)malloc(sizeof(int)*((*parameters)->groups_max+1));
      if (!(*parameters)->groups) goto error;
      memcpy((*parameters)->groups, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->groups_max);
      (*parameters)->groups[(*parameters)->groups_max]=0;
      nbuffer_in+=sizeof(int)*(*parameters)->groups_max;
    }
    if ((*parameters)->message_sizes_max){
      (*parameters)->message_sizes=(int*)malloc(sizeof(int)*((*parameters)->message_sizes_max+1));
      if (!(*parameters)->message_sizes) goto error;
      memcpy((*parameters)->message_sizes, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->message_sizes_max);
    ...
    if ((*parameters)->rank_perm_max){
      (*parameters)->rank_perm=(int*)malloc(sizeof(int)*((*parameters)->rank_perm_max+1));
      if (!(*parameters)->rank_perm) goto error;
      memcpy((*parameters)->rank_perm, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->rank_perm_max);
      (*parameters)->rank_perm[(*parameters)->rank_perm_max]=0;
      nbuffer_in+=sizeof(int)*(*parameters)->rank_perm_max;
    }
    if ((*parameters)->iocounts_max){
      (*parameters)->iocounts=(int*)malloc(sizeof(int)*((*parameters)->iocounts_max+1));
      if (!(*parameters)->iocounts) goto error;
      memcpy((*parameters)->iocounts, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->iocounts_max);
    ...
    if ((*parameters)->shmem_buffer_offset_max){
      (*parameters)->shmem_buffer_offset=(int*)malloc(sizeof(int)*((*parameters)->shmem_buffer_offset_max+1));
      if (!(*parameters)->shmem_buffer_offset) goto error;
      memcpy((*parameters)->shmem_buffer_offset, buffer_in+nbuffer_in, sizeof(int)*(*parameters)->shmem_buffer_offset_max);
      (*parameters)->shmem_buffer_offset[(*parameters)->shmem_buffer_offset_max]=0;
      nbuffer_in+=sizeof(int)*(*parameters)->shmem_buffer_offset_max;
    }
    return nbuffer_in;
  }
  buffer_in_pcopy=strdup(buffer_in);
  if (!buffer_in_pcopy) return ERROR_MALLOC;
  ...
  (*parameters)->root=-1;
  do{
    if (sscanf(buffer_in_copy, "%99s %99s %d", string1, string2, &integer1)>0){
      ...
      if (strcmp(string1, "PARAMETER")==0){
        ...
        if (strcmp(string2, "SHMEM_BUFFER_OFFSET")==0){
          ...
          while ((*buffer_in_copy<'0'||*buffer_in_copy>'9')&&(*buffer_in_copy!='-')&&(*buffer_in_copy!='\0')){
            buffer_in_copy++;
          }
          (*parameters)->shmem_buffer_offset_max=read_int_series(buffer_in_copy, &((*parameters)->shmem_buffer_offset));
          if ((*parameters)->shmem_buffer_offset_max<0) goto error;
        }
        if (strcmp(string2, "COUNTS")==0){
          ...
          (*parameters)->counts_max=read_int_series(buffer_in_copy, &((*parameters)->counts));
          if ((*parameters)->counts_max<0) goto error;
        }
        if (strcmp(string2, "NUM_PORTS")==0){
          ...
          (*parameters)->num_ports_max=read_int_series(buffer_in_copy, &((*parameters)->num_ports));
          if ((*parameters)->num_ports_max<0) goto error;
        }
        if (strcmp(string2, "GROUPS")==0){
          ...
          (*parameters)->groups_max=read_int_series(buffer_in_copy, &((*parameters)->groups));
          if ((*parameters)->groups_max<0) goto error;
        }
        if (strcmp(string2, "MESSAGE_SIZE")==0){
          ...
          (*parameters)->message_sizes_max=read_int_series(buffer_in_copy, &((*parameters)->message_sizes));
          if ((*parameters)->message_sizes_max<0) goto error;
        }
        if (strcmp(string2, "RANK_PERM")==0){
          ...
          (*parameters)->rank_perm_max=read_int_series(buffer_in_copy, &((*parameters)->rank_perm));
          if ((*parameters)->rank_perm_max<0) goto error;
        }
        if (strcmp(string2, "IOCOUNTS")==0){
          ...
          (*parameters)->iocounts_max=read_int_series(buffer_in_copy, &((*parameters)->iocounts));
          if ((*parameters)->iocounts_max<0) goto error;
        }
      }else{
        if (string1[0]!='#'){
          flag=0;
...
int write_parameters(struct parameters_block *parameters, char *buffer_out){
  ...
  if (!parameters->ascii_out){
    ...
    if (parameters->counts_max){
      memcpy(buffer_out+nbuffer_out, parameters->counts, sizeof(int)*parameters->counts_max);
      nbuffer_out+=sizeof(int)*parameters->counts_max;
    }
    if (parameters->num_ports_max){
      memcpy(buffer_out+nbuffer_out, parameters->num_ports, sizeof(int)*parameters->num_ports_max);
      nbuffer_out+=sizeof(int)*parameters->num_ports_max;
    }
    if (parameters->groups_max){
      memcpy(buffer_out+nbuffer_out, parameters->groups, sizeof(int)*parameters->groups_max);
      nbuffer_out+=sizeof(int)*parameters->groups_max;
    }
    if (parameters->message_sizes_max){
      memcpy(buffer_out+nbuffer_out, parameters->message_sizes, sizeof(int)*parameters->message_sizes_max);
      nbuffer_out+=sizeof(int)*parameters->message_sizes_max;
    }
    if (parameters->rank_perm_max){
      memcpy(buffer_out+nbuffer_out, parameters->rank_perm, sizeof(int)*parameters->rank_perm_max);
      nbuffer_out+=sizeof(int)*parameters->rank_perm_max;
    }
    if (parameters->iocounts_max){
      memcpy(buffer_out+nbuffer_out, parameters->iocounts, sizeof(int)*parameters->iocounts_max);
      nbuffer_out+=sizeof(int)*parameters->iocounts_max;
    }
    if (parameters->shmem_buffer_offset_max){
      memcpy(buffer_out+nbuffer_out, parameters->shmem_buffer_offset, sizeof(int)*parameters->shmem_buffer_offset_max);
      nbuffer_out+=sizeof(int)*parameters->shmem_buffer_offset_max;
    }
    return nbuffer_out;
  }
  if (parameters->locmem_max>=0){
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER LOCMEM_MAX %d\n", parameters->locmem_max);
  }
  if (parameters->shmem_max>=0){
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER SHMEM_MAX %d\n", parameters->shmem_max);
  }
  if (parameters->shmem_buffer_offset_max>0){
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER SHMEM_BUFFER_OFFSET");
    for (i=0; i<parameters->shmem_buffer_offset_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->shmem_buffer_offset[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  switch (parameters->collective_type){
    case collective_type_allgatherv:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER COLLECTIVE_TYPE ALLGATHERV\n");
      break;
    case collective_type_reduce_scatter:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER COLLECTIVE_TYPE REDUCE_SCATTER\n");
      break;
    case collective_type_allreduce:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER COLLECTIVE_TYPE ALLREDUCE\n");
      break;
    case collective_type_allreduce_group:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER COLLECTIVE_TYPE ALLREDUCE_GROUP\n");
      break;
    case collective_type_allreduce_short:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER COLLECTIVE_TYPE ALLREDUCE_SHORT\n");
      break;
  }
  ...
  nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER NODE_ROW_SIZE %d\n", parameters->node_row_size);
  nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER NODE_COLUMN_SIZE %d\n", parameters->node_column_size);
  nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER COPY_METHOD %d\n", parameters->copy_method);
  ...
  if (parameters->counts_max>0){
    ...
    for (i=0; i<parameters->counts_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->counts[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  if (parameters->num_ports_max>0){
    ...
    for (i=0; i<parameters->num_ports_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->num_ports[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  if (parameters->groups_max>0){
    ...
    for (i=0; i<parameters->groups_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->groups[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  if (parameters->message_sizes_max>0){
    ...
    for (i=0; i<parameters->message_sizes_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->message_sizes[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  if (parameters->rank_perm_max>0){
    ...
    for (i=0; i<parameters->rank_perm_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->rank_perm[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  if (parameters->iocounts_max>0){
    ...
    for (i=0; i<parameters->iocounts_max; i++){
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", parameters->iocounts[i]);
    }
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  }
  switch (parameters->data_type){
    case data_type_char:
      break;
    case data_type_int:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER DATA_TYPE INT\n");
      break;
    case data_type_long_int:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER DATA_TYPE LONG_INT\n");
      break;
    case data_type_float:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER DATA_TYPE FLOAT\n");
      break;
    case data_type_double:
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER DATA_TYPE DOUBLE\n");
      break;
  }
  if (parameters->bit_identical){
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, " PARAMETER BIT_IDENTICAL\n");
  }
...
static int read_int_tuple_series(char *string_in, int **integers_out_1st, int **integers_out_2nd){
  ...
  *integers_out_1st = (int*) malloc(i_max*sizeof(int));
  if (!*integers_out_1st) goto error;
  *integers_out_2nd = (int*) malloc(i_max*sizeof(int));
  if (!*integers_out_2nd) goto error;
  string_in_new=string_in__;
  for (i=0; i<i_max; i++){
    if (sscanf(string_in_new, "%d %d", &(*integers_out_1st)[i], &(*integers_out_2nd)[i])!=2){
      free(string_in__);
      return(-1);
    }
    for (j=0; j<2; j++){
...
int read_stage_line(char *string_in, struct data_line *data){
  ...
  if (sscanf(string_in, "%99s %d %99s %d %99s %d %99s", string_stage, &stage, string_frac, &frac, string_source, &source, string_to)!=7){
    return ERROR_SYNTAX;
  }
  if (strcmp(string_stage, "STAGE")||strcmp(string_frac, "FRAC")||strcmp(string_source, "SOURCE")||strcmp(string_to, "TO")){
    return ERROR_SYNTAX;
  }
  data->frac=frac;
  data->source=source;
  ...
  if (data->to_max<0) return(data->to_max);
  string_in=strstr(string_in, "FROM ");
  if (!string_in){
    return ERROR_SYNTAX;
  }
  data->from_max=read_int_tuple_series(string_in+strlen("FROM "), &data->from_node, &data->from_line);
  if (data->from_max<0){
    free(data->to);
...
int read_algorithm(char *buffer_in, int *size_level0, int **size_level1, struct data_line ***data, int ascii_in){
  char *line=NULL;
  ...
  *data=NULL;
  ...
  if (!ascii_in){
    ...
    *data=(struct data_line **)malloc(sizeof(struct data_line *)*(*size_level0));
    ...
    for (i=0; i<*size_level0; i++){
      (*data)[i]=(struct data_line *)malloc((*size_level1)[i]*sizeof(struct data_line));
      if (!(*data)[i]) goto error;
      for (j=0; j<(*size_level1)[i]; j++){
        memcpy(&(*data)[i][j], buffer_in+nbuffer_in, sizeof(struct data_line));
        nbuffer_in+=sizeof(struct data_line);
        (*data)[i][j].to=(int*)malloc(sizeof(int)*(*data)[i][j].to_max);
        if (!(*data)[i][j].to) goto error;
        memcpy((*data)[i][j].to, buffer_in+nbuffer_in, sizeof(int)*(*data)[i][j].to_max);
        nbuffer_in+=sizeof(int)*(*data)[i][j].to_max;
        (*data)[i][j].from_node=(int*)malloc(sizeof(int)*(*data)[i][j].from_max);
        if (!(*data)[i][j].from_node) goto error;
        memcpy((*data)[i][j].from_node, buffer_in+nbuffer_in, sizeof(int)*(*data)[i][j].from_max);
        nbuffer_in+=sizeof(int)*(*data)[i][j].from_max;
        (*data)[i][j].from_line=(int*)malloc(sizeof(int)*(*data)[i][j].from_max);
        if (!(*data)[i][j].from_line) goto error;
        memcpy((*data)[i][j].from_line, buffer_in+nbuffer_in, sizeof(int)*(*data)[i][j].from_max);
        nbuffer_in+=sizeof(int)*(*data)[i][j].from_max;
      }
    }
  }else{
...
int write_algorithm(int size_level0, int *size_level1, struct data_line **data, char *buffer_out, int ascii_out){
  ...
  if (!ascii_out){
    ...
    for (i=0; i<size_level0; i++){
      for (j=0; j<size_level1[i]; j++){
        ...
        memcpy(buffer_out+nbuffer_out, data[i][j].to, sizeof(int)*data[i][j].to_max);
        nbuffer_out+=sizeof(int)*data[i][j].to_max;
        memcpy(buffer_out+nbuffer_out, data[i][j].from_node, sizeof(int)*data[i][j].from_max);
        nbuffer_out+=sizeof(int)*data[i][j].from_max;
        memcpy(buffer_out+nbuffer_out, data[i][j].from_line, sizeof(int)*data[i][j].from_max);
        nbuffer_out+=sizeof(int)*data[i][j].from_max;
      }
    }
  }else{
    for (i=0; i<size_level0; i++){
      for (j=0; j<size_level1[i]; j++){
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, " STAGE %d FRAC %d SOURCE %d TO", i, data[i][j].frac, data[i][j].source);
        for (k=0; k<data[i][j].to_max; k++){
          nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", data[i][j].to[k]);
        }
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, " FROM");
        for (k=0; k<data[i][j].from_max; k++){
          nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d|%d", data[i][j].from_node[k], data[i][j].from_line[k]);
        }
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
      }
      nbuffer_out+=sprintf(buffer_out+nbuffer_out, "#\n");
...
void delete_algorithm(int size_level0, int *size_level1, struct data_line **data){
  int i, j;
  for (i=0; i<size_level0; i++){
    if (data){
      if (data[i]){
...
static int write_eassembler_type(char *buffer_out, enum eassembler_type string1, int ascii){
  int nbuffer_out=0;
  if (!ascii){
...
int write_assembler_line_s(char *buffer_out, enum eassembler_type string1, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
  if (!ascii) memcpy(buffer_out, &nbuffer_out, sizeof(nbuffer_out));
  if (ascii) nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
  return nbuffer_out;
}

int write_assembler_line_sd(char *buffer_out, enum eassembler_type string1, int integer1, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
...
int write_assembler_line_sdsd(char *buffer_out, enum eassembler_type string1, int integer1, enum eassembler_type string2, int integer2, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
  ...
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer1, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string2, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
...
int write_assembler_line_ssdsdd(char *buffer_out, enum eassembler_type string1, enum eassembler_type string2, int integer1, enum eassembler_type string3, int integer2, int integer3, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
  ...
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer1, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string3, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer2, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
...
int write_assembler_line_ssdddd(char *buffer_out, enum eassembler_type string1, enum eassembler_type string2, int integer1, int integer2, int integer3, int integer4, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
...
int write_assembler_line_ssdsdddd(char *buffer_out, enum eassembler_type string1, enum eassembler_type string2, int integer1, enum eassembler_type string3, int integer2, int integer3, int integer4, int integer5, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
...
int write_assembler_line_ssdsdsdsdd(char *buffer_out, enum eassembler_type string1, enum eassembler_type string2, int integer1, enum eassembler_type string3, int integer2, enum eassembler_type string4, int integer3, enum eassembler_type string5, int integer4, int integer5, int ascii){
  int nbuffer_out=0;
  if (!ascii) nbuffer_out+=sizeof(int);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string1, ascii);
  ...
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer2, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string4, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer3, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=0;
  nbuffer_out+=write_eassembler_type(buffer_out+nbuffer_out, string5, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer4, ascii);
  if (!ascii) buffer_out[nbuffer_out++]=1;
  nbuffer_out+=write_integer(buffer_out+nbuffer_out, integer5, ascii);
...
int read_assembler_line_s(char *buffer_in, enum eassembler_type *string1, int ascii){
  ...
  if (!ascii){
    ...
    i+=sizeof(n);
    if (buffer_in[i++]!=0) return -11;
    if (i<n) memcpy(string1, buffer_in+i, sizeof(*string1));
    i+=sizeof(*string1);
    if (i==n){
      return n;
    }else{
...
int read_assembler_line_sd(char *buffer_in, enum eassembler_type *string1, int *integer1, int ascii){
  char cstring1[100], cstring2[100];
  int n, i;
  if (!ascii){
    i=0;
    ...
    if (i<n) memcpy(string1, buffer_in+i, sizeof(*string1));
    i+=sizeof(*string1);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer1, buffer_in+i, sizeof(*integer1));
    i+=sizeof(*integer1);
    if (i==n){
      return n;
    }else{
...
int read_assembler_line_sdsd(char *buffer_in, enum eassembler_type *string1, int *integer1, enum eassembler_type *string2, int *integer2, int ascii){
  ...
  if (!ascii){
    ...
    i+=sizeof(*integer1);
    if (buffer_in[i++]!=0) return -11;
    if (i<n) memcpy(string2, buffer_in+i, sizeof(*string2));
    i+=sizeof(*string2);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer2, buffer_in+i, sizeof(*integer2));
    i+=sizeof(*integer2);
    if (i==n){
      return n;
    }else{
    ...
    n=sscanf(buffer_in, "%99s %d %99s %d %99s", cstring1, integer1, cstring2, integer2, cstring3);
    if (n<1){
      return -11;
    }
    *string1=read_assembler_type(cstring1);
...
int read_assembler_line_ssdsdd(char *buffer_in, enum eassembler_type *string1, enum eassembler_type *string2, int *integer1, enum eassembler_type *string3, int *integer2, int *integer3, int ascii){
  ...
  if (!ascii){
    ...
    i+=sizeof(*integer1);
    if (buffer_in[i++]!=0) return -11;
    if (i<n) memcpy(string3, buffer_in+i, sizeof(*string3));
    i+=sizeof(*string3);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer2, buffer_in+i, sizeof(*integer2));
    i+=sizeof(*integer2);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer3, buffer_in+i, sizeof(*integer3));
    i+=sizeof(*integer3);
    if (i==n){
      return n;
    }else{
    ...
    n=sscanf(buffer_in, "%99s %99s %d %99s %d %d %99s", cstring1, cstring2, integer1, cstring3, integer2, integer3, cstring4);
    if (n<1){
      return -11;
    }
    *string1=read_assembler_type(cstring1);
...
int read_assembler_line_ssdddd(char *buffer_in, enum eassembler_type *string1, enum eassembler_type *string2, int *integer1, int *integer2, int *integer3, int *integer4, int ascii){
  char cstring1[100], cstring2[100], cstring3[100];
  int n, i;
  if (!ascii){
    i=0;
    ...
    i+=sizeof(*integer3);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer4, buffer_in+i, sizeof(*integer4));
    i+=sizeof(*integer4);
    if (i==n){
      return n;
    }else{
    ...
    n=sscanf(buffer_in, "%99s %99s %d %d %d %d %99s", cstring1, cstring2, integer1, integer2, integer3, integer4, cstring3);
    if (n<1){
      return -11;
    }
    *string1=read_assembler_type(cstring1);
...
int read_assembler_line_ssdsdddd(char *buffer_in, enum eassembler_type *string1, enum eassembler_type *string2, int *integer1, enum eassembler_type *string3, int *integer2, int *integer3, int *integer4, int *integer5, int ascii){
  char cstring1[100], cstring2[100], cstring3[100], cstring4[100];
  int n, i;
  if (!ascii){
    i=0;
    ...
    i+=sizeof(*integer4);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer5, buffer_in+i, sizeof(*integer5));
    i+=sizeof(*integer5);
    if (i==n){
      return n;
    }else{
    ...
    n=sscanf(buffer_in, "%99s %99s %d %99s %d %d %d %d %99s", cstring1, cstring2, integer1, cstring3, integer2, integer3, integer4, integer5, cstring4);
    if (n<1){
      return -11;
    }
    *string1=read_assembler_type(cstring1);
...
int read_assembler_line_ssdsdsdsdd(char *buffer_in, enum eassembler_type *string1, enum eassembler_type *string2, int *integer1, enum eassembler_type *string3, int *integer2, enum eassembler_type *string4, int *integer3, enum eassembler_type *string5, int *integer4, int *integer5, int ascii){
  char cstring1[100], cstring2[100], cstring3[100], cstring4[100], cstring5[100], cstring6[100];
  int n, i;
  if (!ascii){
    i=0;
    memcpy(&n, buffer_in, sizeof(n));
    ...
    i+=sizeof(*integer2);
    if (buffer_in[i++]!=0) return -11;
    if (i<n) memcpy(string4, buffer_in+i, sizeof(*string4));
    i+=sizeof(*string4);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer3, buffer_in+i, sizeof(*integer3));
    i+=sizeof(*integer3);
    if (buffer_in[i++]!=0) return -11;
    if (i<n) memcpy(string5, buffer_in+i, sizeof(*string5));
    i+=sizeof(*string5);
    if (buffer_in[i++]!=1) return -11;
    if (i<n) memcpy(integer4, buffer_in+i, sizeof(*integer4));
    i+=sizeof(*integer4);
    ...
    n=sscanf(buffer_in, "%99s %99s %d %99s %d %99s %d %99s %d %d %99s", cstring1, cstring2, integer1, cstring3, integer2, cstring4, integer3, cstring5, integer4, integer5, cstring6);
    if (n<1){
      return -11;
    }
    *string1=read_assembler_type(cstring1);
