...
int ext_mpi_generate_optimise_buffers2(char *buffer_in, char *buffer_out) {
  int nbuffer_out = 0, nbuffer_in = 0, nbuffer_out_start, flag, flag2, flag3, o1, o2, size, o1_,
      size_, partner, num_comm, nline2, round=0, flag_loop, i;
  ...
  if (buffer_temp == NULL) goto error;
  do {
  flag_loop=0;
  if (round==0){
  ...
  do {
    ...
    if (flag3) {
      if (read_assembler_line_s(line, &estring1, 0) >= 0) {
        flag = 1;
        if (read_line(buffer_in_loop + nbuffer_in, line2, parameters->ascii_in)) {
                      ...
                      if (read_assembler_line_ssdsdd(line2, &estring1,
                        ...
                        if ((o2 == o1_) && (size == size_)) {
                          nbuffer_out += write_assembler_line_ssdddd(
                              buffer_out_loop + nbuffer_out, estring1_, estring2_,
                              o1, size_, partner, num_comm,
        ...
        if (flag && (nbuffer_in != nline2)) {
          nbuffer_out +=
              write_line(buffer_out_loop + nbuffer_out, line, parameters->ascii_out);
        }
      }
    }
  } while (flag3);
  ...
  if (!(round%2)){
    ...
    nbuffer_in=nbuffer_out=0;
    do{
      nbuffer_in+=flag=read_line(buffer_in_loop + nbuffer_in, line, parameters->ascii_in);
      if (flag){
