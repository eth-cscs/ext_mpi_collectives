...
int generate_buffer_offset(char *buffer_in, char *buffer_out){
  int nbuffer_out=0, nbuffer_in=0, nbuffer_in2=0, bo1, bo2, o1, o2, size, *buffer_offset=NULL, *buffer_offset2=NULL, buffer_offset_max=0, locmem_max=0, flag, i, shmem_max=0;
  char line[1000];
  ...
  do{
    nbuffer_in2+=flag=read_line(buffer_in+nbuffer_in2, line, parameters->ascii_in);
    if (read_assembler_line_s(line, &estring1, 0)>=0){
      if ((estring1==ememcpy)||(estring1==ereduce)||(estring1==ememcp_)||(estring1==ereduc_)||(estring1==esreduce)||(estring1==esreduc_)){
  ...
  if (!buffer_offset) goto error;
  buffer_offset2=(int*)malloc(sizeof(int)*buffer_offset_max);
  if (!buffer_offset2) goto error;
