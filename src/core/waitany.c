    ...
    if (flag) {
      if (read_assembler_line_s(line, &estring1, 0) >= 0) {
        if (estring1 == ewaitall) {
          ...
          flag3 = 0;
          do{
            flag3+=read_line(buffer_in + nbuffer_in + flag + flag3, line, parameters->ascii_in);
            read_assembler_line_s(line, &estring1_, 0);
            if (estring1_ == ereturn){
              flag3 = 0;
            } else if (estring1_ == ereduce){
              read_assembler_line_ssdsdd(line, &estring1_, &estring2_, &o1_, &estring3_, &o2_, &size_, 0);
              flag3 = 0;
              if (o1__==o1_){
                nreduce = -1;
              }
	  ...
	  flag2=1;
	    ...
	    flag2=0;
          ...
          nbuffer_out += write_line(buffer_out + nbuffer_out, line,
                                    parameters->ascii_out);
          if (estring1 == eisend){
            read_assembler_line_ssdddd(line, &estring1, &estring2, &o1__, &size__, &partner, &num_comm, 0);
          }
