...
static int allreduce_start(struct allreduce_data_element **stages, int num_processors, int *num_ports, int task, int allreduce){
  ...
  *stages = (struct allreduce_data_element *)malloc((nsteps+1)*sizeof(struct allreduce_data_element));
  if (!(*stages)) goto error;
  ...
  if (!stages_in){
    for (step=0; step<=nsteps; step++){
      if (allreduce){
        if (allreduce==3){
          for ((*stages)[step].max_lines=1, i=1; (num_ports[i-1]>0)&&(i<=step); (*stages)[step].max_lines=((*stages)[step].max_lines-1)/(num_ports[i-1]+1)+1, i++){}
        }else{
          for ((*stages)[step].max_lines=num_processors, i=1; (num_ports[i-1]>0)&&(i<=step); (*stages)[step].max_lines=((*stages)[step].max_lines-1)/(num_ports[i-1]+1)+1, i++){}
        }
      }else{
      ...
      for (; num_ports[i-1]&&(i<=step); (*stages)[step].max_lines*=(abs(num_ports[i-1])+1), i++){}
      ...
      (*stages)[step].frac = (int*) malloc(sizeof(int)*(*stages)[step].max_lines);
      if (!(*stages)[step].frac) goto error;
      (*stages)[step].source = (int*) malloc(sizeof(int)*(*stages)[step].max_lines);
      if (!(*stages)[step].source) goto error;
#ifdef VERBOSE
      (*stages)[step].num_sources = (int*) malloc(sizeof(int)*(*stages)[step].max_lines);
      if (!(*stages)[step].num_sources) goto error;
      (*stages)[step].sources_value = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*num_processors);
      if (!(*stages)[step].sources_value) goto error;
#endif
      (*stages)[step].num_from = (int*) malloc(sizeof(int)*(*stages)[step].max_lines);
      if (!(*stages)[step].num_from) goto error;
      (*stages)[step].from_value = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*max_port_from);
      if (!(*stages)[step].from_value) goto error;
      (*stages)[step].from_line = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*max_port_from);
      if (!(*stages)[step].from_line) goto error;
      (*stages)[step].num_to = (int*) malloc(sizeof(int)*(*stages)[step].max_lines);
      if (!(*stages)[step].num_to) goto error;
      (*stages)[step].to_value = (int*) malloc(sizeof(int)*(*stages)[step].max_lines*max_port_to);
      if (!(*stages)[step].to_value) goto error;
    }
...
static void allreduce_done(struct allreduce_data_element *stages, int *num_ports){
  int nsteps, step;
  for (nsteps=0; num_ports[nsteps]; nsteps++){}
  for (step=0; step<=nsteps; step++){
    free(stages[step].to_value);
...
#ifdef VERBOSE
static int allreduce_complete(struct allreduce_data_element *stages, int step, int num_processors, int line){
  return(stages[step].num_sources[line]==num_processors);
}

static int allreduce_independent(struct allreduce_data_element *stages, int step, int num_processors, int line1, int line2){
  int i, j;
  ...
  if (stages[step].num_sources[line1]+stages[step].num_sources[line2]>num_processors){
    return 0;
  }
  memset(temp_array, 0, num_processors);
  for (i=0; i<stages[step].num_sources[line1]; i++){
    temp_array[stages[step].sources_value[i*stages[step].max_lines+line1]]=1;
  }
  for (j=0; j<stages[step].num_sources[line2]; j++){
    if (temp_array[stages[step].sources_value[j*stages[step].max_lines+line2]]){
      return 0;
    }
  }
  return 1;
...
static int update_lines_used(struct allreduce_data_element *stages, int *num_ports, int *lines_used){
  ...
  for (step--; (step>=0)&&(num_ports[step]<0); step--){
    for (port=0; port<abs(num_ports[step]); port++){
      for (i=0; (i<stages[step].max_lines)&&(i+(port+1)*stages[step].max_lines<stages[step+1].max_lines); i++){
...
static int allreduce_init(struct allreduce_data_element **stages, int num_processors, int *num_ports, int *lines_used, int task, int allreduce){
  int chunk, gbstep, port, i, j, k, l, m, step, task_dup, step_mid, flag, im, *used=NULL, used_max;
  ...
  for (step=0; num_ports[step]>0; step++){
    for (i=0, gbstep=num_processors; (num_ports[i]>0)&&(i<=step); i++){
    ...
    for (i=0; i<gbstep*(*stages)[0].max_lines/num_processors; i++){
      ...
      for (j=0; j<(*stages)[step+1].num_sources[i]; j++){
        (*stages)[step+1].sources_value[j*(*stages)[step+1].max_lines+i] = (*stages)[step].sources_value[j*(*stages)[step].max_lines+i];
      }
#endif
    }
    for (port=0; port<num_ports[step]; port++){
      for (i=0; (i<gbstep*(*stages)[0].max_lines/num_processors)&&((port+1)*gbstep*(*stages)[0].max_lines/num_processors+i<(*stages)[step].max_lines); i++){
        (*stages)[step+1].num_from[i] = 1;
        (*stages)[step+1].from_value[i] = task;
    ...
    for (port=0; port<num_ports[step]; port++){
      ...
      for (i=0; (i<gbstep*(*stages)[0].max_lines/num_processors)&&((port+1)*gbstep*(*stages)[0].max_lines/num_processors+i<(*stages)[step].max_lines); i++){
        (*stages)[step+1].frac[i] = ((*stages)[0].max_lines+(*stages)[step].frac[gbstep*(port+1)*(*stages)[0].max_lines/num_processors+i]+(task_dup-task)*(*stages)[0].max_lines/num_processors)%(*stages)[0].max_lines;
        (*stages)[step+1].source[i] = task;
        (*stages)[step+1].from_value[(*stages)[step+1].max_lines*(*stages)[step+1].num_from[i]+i] = task_dup;
        (*stages)[step+1].from_line[(*stages)[step+1].max_lines*(*stages)[step+1].num_from[i]+i] = (port+1)*gbstep+i;
        (*stages)[step+1].num_from[i]++;
#ifdef VERBOSE
        for (j=0; j<(*stages)[step].num_sources[gbstep*(port+1)+i]; j++){
          (*stages)[step+1].sources_value[((*stages)[step+1].num_sources[i]+j)*(*stages)[step+1].max_lines+i] = (num_processors+(*stages)[step].sources_value[j*(*stages)[step].max_lines+i]+task_dup-task)%num_processors;
        }
        (*stages)[step+1].num_sources[i] += (*stages)[step].num_sources[gbstep*(port+1)+i];
#endif
      }
    }
  }
  ...
  for (step=step_mid; num_ports[step]; step++){
    ...
    for (port=0; port<abs(num_ports[step]); port++){
      task_dup = (num_processors+task+(port+1)*gbstep)%num_processors;
      for (i=0; (i<chunk)&&(chunk*(port+1)+i<(*stages)[step+1].max_lines); i++){
        (*stages)[step+1].num_from[chunk*(port+1)+i] = 1;
        (*stages)[step+1].from_value[chunk*(port+1)+i] = (num_processors+task_dup)%num_processors;
//        (*stages)[step+1].from_line[chunk*(port+1)+i] = chunk*(port+1)+i;
        (*stages)[step+1].from_line[chunk*(port+1)+i] = chunk*port+i;
        (*stages)[step+1].frac[chunk*(port+1)+i] = ((*stages)[0].max_lines+(*stages)[step].frac[i]+(task_dup-task)*(*stages)[0].max_lines/num_processors)%(*stages)[0].max_lines;
        if (!allreduce){
          (*stages)[step+1].frac[chunk*(port+1)+i] = (num_processors*(*stages)[0].max_lines+(*stages)[step].frac[i]+(task_dup-task)*(*stages)[0].max_lines)%(num_processors*(*stages)[0].max_lines);
        }
        (*stages)[step+1].source[chunk*(port+1)+i] = (num_processors+(*stages)[step].source[i]+task_dup-task)%num_processors;
#ifdef VERBOSE
        for (j=0; j<(*stages)[step].num_sources[i]; j++){
          (*stages)[step+1].sources_value[(*stages)[step+1].max_lines*j+chunk*(port+1)+i] = (num_processors+(*stages)[step].sources_value[(*stages)[step].max_lines*j+i]+task_dup-task)%num_processors;
        }
        (*stages)[step+1].num_sources[chunk*(port+1)+i] = (*stages)[step].num_sources[i];
#endif
      }
    }
    for (port=0; port<abs(num_ports[step]); port++){
      ...
      for (i=0; i<im; i++){
        ...
        for (j=(port+1)*chunk-1; (j>=0)&&used[(*stages)[step+1].frac[j]]; j--){
          if ((*stages)[step+1].frac[j]==(*stages)[step+1].frac[chunk*(port+1)+i]){
#ifdef VERBOSE
            if (!allreduce_independent(*stages, step, num_processors, chunk*(port+1)+i, j)){
              printf("lines not independent %d %d %d\n", step, chunk*(port+1)+i, j);
              exit(2);
            }
            for (k=0; k<(*stages)[step+1].num_sources[j]; k++){
              (*stages)[step+1].sources_value[(*stages)[step+1].max_lines*(k+(*stages)[step+1].num_sources[chunk*(port+1)+i])+chunk*(port+1)+i] = (*stages)[step+1].sources_value[(*stages)[step+1].max_lines*k+j];
            }
            (*stages)[step+1].num_sources[chunk*(port+1)+i] += (*stages)[step+1].num_sources[j];
#endif
            (*stages)[step+1].from_value[(*stages)[step+1].max_lines*(*stages)[step+1].num_from[chunk*(port+1)+i]+chunk*(port+1)+i] = task;
            (*stages)[step+1].from_line[(*stages)[step+1].max_lines*(*stages)[step+1].num_from[chunk*(port+1)+i]+chunk*(port+1)+i] = j;
    ...
    for (step=step_mid+1; num_ports[step-1]; step++){
      for (i=0; i<(*stages)[step].max_lines; i++){
        for (k=0; k<(*stages)[step].num_from[i]; k++){
          ...
          if ((k>0)&&(m>=0)&&((*stages)[step].from_value[(*stages)[step].max_lines*k+i]==task)){
            if (!lines_used[m]){
              flag=1;
              lines_used[m] = 1;
            }
  ...
  for (step=step_mid+1; num_ports[step-1]; step++){
    ...
    for (port=0; port<abs(num_ports[step-1]); port++){
      for (i=0; (i<chunk)&&(chunk*(port+1)+i<(*stages)[step].max_lines); i++){
        for (k=0; k<(*stages)[step].num_from[chunk*(port+1)+i]; k++){
          if ((*stages)[step].from_line[(*stages)[step].max_lines*k+chunk*(port+1)+i]>=0){
            j=l=(*stages)[step].from_line[(*stages)[step].max_lines*k+chunk*(port+1)+i];
            for (m=0; m<j; m++){
              if (!lines_used[m]){
                l--;
              }
            }
            (*stages)[step].from_line[(*stages)[step].max_lines*k+chunk*(port+1)+i] = l;
          }
        }
      }
    }
  ...
  if (allreduce!=2){
    ...
    for (step=0; num_ports[step]>0; step++){
      ...
      for (port=0; port<num_ports[step]; port++){
        for (i=0; (i<chunk)&&(i+(port+1)*chunk<(*stages)[step].max_lines); i++){
          (*stages)[step].num_to[i+(port+1)*chunk] = 1;
          (*stages)[step].to_value[i+(port+1)*chunk] = (num_processors+task+(port+1)*gbstep)%num_processors;
        }
      }
    }
  }else{
    gbstep=num_processors;
    for (step=0; num_ports[step]>0; step++){
      chunk = (((*stages)[step].max_lines/((*stages)[0].max_lines/num_processors)-1)/(num_ports[step]+1)+1)*((*stages)[0].max_lines/num_processors);
      gbstep = (gbstep-1)/(num_ports[step]+1)+1;
      for (i=0; i<chunk; i++){
        (*stages)[step].num_to[i] = 1;
        (*stages)[step].to_value[i] = task;
  ...
  for (; num_ports[step]; step++){
    for (i=0; i<(*stages)[step].max_lines; i++){
      ...
      (*stages)[step].to_value[i] = task;
      for (port=0; port<abs(num_ports[step]); port++){
        if ((port+1)*(*stages)[step].max_lines+i<(*stages)[step+1].max_lines){
          if (lines_used[(port+1)*(*stages)[step].max_lines+i]){
            (*stages)[step].to_value[(*stages)[step].max_lines*(*stages)[step].num_to[i]+i] = (num_processors+task-(port+1)*gbstep)%num_processors;
...
int generate_allreduce(char *buffer_in, char *buffer_out){
  ...
  if (size_level0){
    stages=(struct allreduce_data_element *)malloc(sizeof(struct allreduce_data_element)*(nstep+1));
    if (!stages) goto error;
    for (i=0; i<=nstep; i++){
  ...
  if (!lines_used) return ERROR_MALLOC;
  if (allreduce){
  ...
  i=allreduce_init(&stages, num_nodes, num_ports, lines_used, task, allreduce);
  if (i<0) goto error;
  if (size_level0){
  ...
  for (i=0; i<=nstep; i++){
    for (j=0; j<stages[i].max_lines; j++){
      ...
      if (lines_used[j]||verbose||(i==0)||(num_ports[i-1]>0)){
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, " FRAC %d", stages[i].frac[j]);
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, " SOURCE %d", stages[i].source[j]);
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, " TO");
        for (k=0; k<stages[i].num_to[j]; k++){
          nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", stages[i].to_value[k*stages[i].max_lines+j]);
        }
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, " FROM");
        for (k=0; k<stages[i].num_from[j]; k++){
          nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d|%d", stages[i].from_value[k*stages[i].max_lines+j], stages[i].from_line[k*stages[i].max_lines+j]);
        }
        if (verbose){
          nbuffer_out+=sprintf(buffer_out+nbuffer_out, " #");
          for (k=0; k<stages[i].num_sources[j]; k++){
            nbuffer_out+=sprintf(buffer_out+nbuffer_out, " %d", stages[i].sources_value[k*stages[i].max_lines+j]);
          }
        }
        nbuffer_out+=sprintf(buffer_out+nbuffer_out, "\n");
      }
  ...
  for (i=0; i<size_level0; i++){
    ...
    data[i]=(struct data_line *)malloc(sizeof(struct data_line)*size_level1[i]);
  ...
  for (i=0; i<size_level0; i++){
    for (j=0; j<size_level1[i]; j++){
      for (k=0; k<data[i][j].from_max; k++){
        if ((data[i][j].from_node[k]==task)&&(data[i][j].from_line[k]>j)){
