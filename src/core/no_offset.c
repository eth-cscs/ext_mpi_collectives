...
int ext_mpi_generate_no_offset(char *buffer_in, char *buffer_out) {
  ...
  int nbuffer_out = 0, nbuffer_in = 0, *buffer_offset, flag;
  char line[1000];
        ...
        if ((estring1 == eirecv || estring1 == eisend) &&
          ...
          if (data_irecv_isend.is_offset) {
            ...
            if (data_irecv_isend.offset_number >= 0) {
              data_irecv_isend.offset += buffer_offset[data_irecv_isend.offset_number + parameters->shmem_buffer_offset_max / parameters->socket_row_size * data_irecv_isend.buffer_number];
            } else {
          ...
          if (data_memcpy_reduce.is_offset1) {
            ...
            if (data_memcpy_reduce.offset_number1 >= 0) {
              data_memcpy_reduce.offset1 += buffer_offset[data_memcpy_reduce.offset_number1 + parameters->shmem_buffer_offset_max / parameters->socket_row_size * data_memcpy_reduce.buffer_number1];
            } else {
          ...
          if (data_memcpy_reduce.is_offset2) {
            ...
            if (data_memcpy_reduce.offset_number2 >= 0) {
              data_memcpy_reduce.offset2 += buffer_offset[data_memcpy_reduce.offset_number2 + parameters->shmem_buffer_offset_max / parameters->socket_row_size * data_memcpy_reduce.buffer_number2];
            } else {
