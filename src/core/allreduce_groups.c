...
static int node_global(int node_local, int gbstep, int factor, int component){
  return(node_local*gbstep+component%gbstep+(component/gbstep)*factor*gbstep);
}

static int insert_lines(int node, int ngroups, int *size_level0, int **size_level1, struct data_line ***data){
  struct data_line *data_new;
  int group, shift, step, i, j;
  for (group=1; group<ngroups; group++){
    shift=size_level1[group-1][size_level0[group-1]-1]-size_level1[group][0];
    for (step=1; step<size_level0[group]; step++){
      data_new=(struct data_line *)malloc((size_level1[group][step]+shift)*sizeof(struct data_line));
      if (!data_new) goto error;
      ...
      for (i=0; i<shift; i++){
        ...
        data_new[i].source=data[group-1][size_level0[group-1]-1][i].source;
        data_new[i].to_max=data[group-1][size_level0[group-1]-1][i].to_max;
        data_new[i].from_max=data[group-1][size_level0[group-1]-1][i].from_max;
        data_new[i].to=(int*)malloc(data_new[i].to_max*sizeof(int));
        if (!data_new[i].to) goto error;
        for (j=0; j<data_new[i].to_max; j++){
          data_new[i].to[j]=data[group-1][size_level0[group-1]-1][i].to[j];
        }
        data_new[i].from_node=(int*)malloc(data_new[i].from_max*sizeof(int));
        if (!data_new[i].from_node) goto error;
        data_new[i].from_line=(int*)malloc(data_new[i].from_max*sizeof(int));
        if (!data_new[i].from_line) goto error;
        data_new[i].from_max=0;
        for (j=0; j<data[group-1][size_level0[group-1]-1][i].from_max; j++){
          if (data[group-1][size_level0[group-1]-1][i].from_node[j]==node){
            data_new[i].from_node[data_new[i].from_max]=data[group-1][size_level0[group-1]-1][i].from_node[j];
            data_new[i].from_line[data_new[i].from_max]=data[group-1][size_level0[group-1]-1][i].from_line[j];
            data_new[i].from_max++;
          }
        }
      }
      for (i=0; i<size_level1[group][step]; i++){
        ...
        data_new[i+shift].to=(int*)malloc(data_new[i+shift].to_max*sizeof(int));
        if (!data_new[i+shift].to) goto error;
        for (j=0; j<data_new[i+shift].to_max; j++){
          data_new[i+shift].to[j]=data[group][step][i].to[j];
        }
        data_new[i+shift].from_node=(int*)malloc(data_new[i+shift].from_max*sizeof(int));
        if (!data_new[i+shift].from_node) goto error;
        data_new[i+shift].from_line=(int*)malloc(data_new[i+shift].from_max*sizeof(int));
        if (!data_new[i+shift].from_line) goto error;
        for (j=0; j<data_new[i+shift].from_max; j++){
          data_new[i+shift].from_node[j]=data[group][step][i].from_node[j];
          data_new[i+shift].from_line[j]=data[group][step][i].from_line[j]+shift;
        }
      }
      for (i=0; i<size_level1[group][step]; i++){
...
static void gen_frac_start(int num_nodes, int node, int *groups, int *frac_start){
  ...
  for (group=0; groups_[group]&&(n>1); group++){
    for (i=0; i<((node/(num_nodes/n))%groups_[group])*(n/groups_[group]); i++){
...
static int frac_translate(int num_nodes, int *groups, int node_from, int node_to, int frac){
...
static int print_algorithm(char *buffer, int node, int num_nodes_l, int node_rank, int node_row_size, int node_column_size, int copy_method, int *counts, int counts_max, int *num_ports, int *msizes, int msizes_max, enum edata_type data_type, int num_nodes, int size_level0, int *size_level1, struct data_line **data, int stage_offset, int size_level0_old, int *size_level1_old, struct data_line **data_old, int allgatherv, int ascii_out, int root, int bit_identical){
  ...
  if (stage_offset==0){
    ...
    for (parameters.num_ports_max=0; num_ports[parameters.num_ports_max]; parameters.num_ports_max++);
    ...
    if (size_level1){
      for (i=0; i<size_level1[0]; i++){
        nbuffer+=sprintf(buffer+nbuffer, " STAGE %d FRAC %d SOURCE %d TO", 0, data[0][i].frac, data[0][i].source);
        ...
        if (stage_offset>0){
          for (j=0; j<data_old[size_level0_old-1][i].from_max; j++){
            nbuffer+=sprintf(buffer+nbuffer, " %d|%d", data_old[size_level0_old-1][i].from_node[j], data_old[size_level0_old-1][i].from_line[j]);
          }
        }else{
          for (j=0; j<data[0][i].from_max; j++){
            nbuffer+=sprintf(buffer+nbuffer, " %d|%d", data[0][i].from_node[j], data[0][i].from_line[j]);
          }
        }
  ...
  for (stage=1; stage<size_level0; stage++){
    for (i=0; i<size_level1[stage]; i++){
      nbuffer+=sprintf(buffer+nbuffer, " STAGE %d FRAC %d SOURCE %d TO", stage+stage_offset, data[stage][i].frac, data[stage][i].source);
      ...
      if ((stage==0)&&(stage_offset>0)){
        ...
        for (j=0; j<data[stage][i].from_max; j++){
          nbuffer+=sprintf(buffer+nbuffer, " %d|%d", data[stage][i].from_node[j], data[stage][i].from_line[j]);
        }
      }
      nbuffer+=sprintf(buffer+nbuffer, "\n");
    }
  ...
  for (i=0; i<num_nodes; i++){
    nbuffer+=sprintf(buffer+nbuffer, " STAGE 0 FRAC %d SOURCE %d TO 0 FROM -1|-1\n", i, node);
  }
  return nbuffer;
}

int generate_allreduce_groups(char *buffer_in, char *buffer_out){
  int node, num_nodes, flag, flag2, node_rank, node_row_size=1, node_column_size=1, stage_offset, copy_method=0;
  int nbuffer_out=0, nbuffer_in=0, *msizes=NULL, msizes_max=0, *msizes_l=NULL, msizes_max_l=0, *num_ports=NULL, num_port, num_ports_max=0, *groups=NULL, group, ngroups=0, igroup, *num_ports_l=NULL, num_nodes_start;
  int i, j, k, l, m, n, o, q, *size_level0_l=NULL, **size_level1_l=NULL, gbstep, component, ports_chunk, node_l, num_nodes_l, *counts=NULL, counts_max, stage=-1;
  int *frac_start=NULL, *frac_temp=NULL, node_o=-1, node_ol=-1, flaggb, node_g, comp_t, gbstep_old, igroup_old;
  ...
  for (group=0; group<ngroups; group++){
    ...
    if (!msizes_l) return ERROR_MALLOC;
    for (i=0; i<msizes_max_l; i++){
      msizes_l[i]=11;
    }
    print_algorithm(buffer_in_temp[group], node_l, num_nodes_l, node_rank, node_row_size, node_column_size, copy_method, counts, counts_max, num_ports_l, msizes_l, msizes_max_l, parameters->data_type, num_nodes_start, 0, NULL, NULL, 0, 0, NULL, NULL, flag, parameters->ascii_out, parameters->root, parameters->bit_identical);
    free(msizes_l);
    if (num_ports_l[0]>0){
    ...
    if (i<0) goto error;
    i=read_algorithm(buffer_out_temp[group]+i, &size_level0_l[group], &size_level1_l[group], &data_l[group], parameters2->ascii_in);
  ...
  for (group=0; group<ngroups; group++){
    ...
    for (stage=0; stage<size_level0_l[group]; stage++){
      for (i=0; i<size_level1_l[group][stage]; i++){
        data_l[group][stage][i].source=node_global(data_l[group][stage][i].source, gbstep, igroup, component);
      }
    }
    gbstep*=abs(groups[num_port]);
    num_port+=ports_chunk;
        ...
        if (stage>1){
          ...
          if (group>0){
            if (i<size_level1_l[group][0]){
              data_l[group][stage][i].frac=data_l[group-1][size_level0_l[group-1]-1][i-size_level1_l[group][0]+size_level1_l[group-1][size_level0_l[group-1]-1]].frac;
              data_l[group][stage][i].source=data_l[group-1][size_level0_l[group-1]-1][i-size_level1_l[group][0]+size_level1_l[group-1][size_level0_l[group-1]-1]].source;
            }
          }
        }
      }
      ...
      for (i=0; i<size_level1_l[group][stage]; i++){
        ...
        for (j=0; (j<data_l[group][stage][i].from_max)&&flag2; j++){
          if ((data_l[group][stage][i].from_node[j]==node_l)&&(j==0)){
            ...
            node_o=(igroup+node_l+(igroup+node_l-data_l[group][stage][i].from_node[j])%igroup)%igroup;
          }
        }
        if (flag){
          n=0;
          ...
          if (stage>1){
            for (k=0; (k<size_level1_l[group][stage-1])&&flag2; k++){
              flag=1;
              for (l=0; (l<data_l[group][stage-1][k].to_max)&&flag2&&flag; l++){
            ...
            if (!flag2){
              node_g=node_global(data_l[group][stage][i].from_node[0], gbstep, igroup, component);
              o=frac_translate(num_nodes, groups, node_g, node, data_l[group][stage-1][k-1].frac);
            }
          }else{
            if (group>0){
              for (k=0; (k<size_level1_l[group-1][size_level0_l[group-1]-1])&&flag2; k++){
                flag=1;
                for (l=0; (l<data_l[group-1][size_level0_l[group-1]-1][k].to_max)&&flag2&&flag; l++){
                  node_local(node, gbstep_old, igroup_old, &comp_t);
                  if (data_l[group-1][size_level0_l[group-1]-1][k].to[l]==-1){
                    m--;
                    flag=0;
                  }
                }
              ...
              if (!flag2){
                node_g=node_global(data_l[group][stage][i].from_node[0], gbstep, igroup, component);
                o=frac_translate(num_nodes, groups, node_g, node, data_l[group-1][size_level0_l[group-1]-1][k-1].frac);
              }
            }
          }
          if (o>=0){
...
//              data_l[group][stage][i].from_line[j]=i;
              flag=1;
              if (j>0){
                for (k=i-1; (k>=0)&&(data_l[group][stage][i].from_node[0]==data_l[group][stage][k].from_node[0]); k--);
              }else{
                k=n-1;
              }
              for (; (k>=0)&&flag; k--){
                if (data_l[group][stage][i].frac==data_l[group][stage][k].frac){
      ...
      for (i=0; i<size_level1_l[group][stage]; i++){
        for (j=0; j<data_l[group][stage][i].to_max; j++){
          if (data_l[group][stage][i].to[j]>=0){
            data_l[group][stage][i].to[j]=node_global(data_l[group][stage][i].to[j], gbstep, igroup, component);
          }
        }
        for (j=0; j<data_l[group][stage][i].from_max; j++){
          if (data_l[group][stage][i].from_node[j]>=0){
            data_l[group][stage][i].from_node[j]=node_global(data_l[group][stage][i].from_node[j], gbstep, igroup, component);
          }
        }
      }
      for (i=0; i<size_level1_l[group][stage]; i++){
//        data_l[group][stage][i].source=node_global(data_l[group][stage][i].source, gbstep, igroup, component);
  ...
  if (parameters->bit_identical&&(group>0)){
    ...
    for (k=0; k<num_nodes; k++){
      ...
      if (flag){
        ...
        l=INT_MAX;
        for (i=0; i<size_level1_l[group][stage]; i++){
          if ((data_l[group][stage][i].frac==k)&&(data_l[group][stage][i].source<l)){
            j=i;
    ...
    if (group<ngroups-1){
      ...
      for (i=0; i<size_level1_l[group][size_level0_l[group]-1]; i++){
        ...
        if (flag2&&(k<size_level1_l[group+1][0])){
          ...
          data_l[group][size_level0_l[group]-1][i].to=(int*)malloc(data_l[group+1][0][k].to_max*sizeof(int));
          if (!data_l[group][size_level0_l[group]-1][i].to) goto error;
          data_l[group][size_level0_l[group]-1][i].to_max=0;
          for (j=0; j<data_l[group+1][0][k].to_max; j++){
            data_l[group][size_level0_l[group]-1][i].to[data_l[group][size_level0_l[group]-1][i].to_max++]=data_l[group+1][0][k].to[j];
          }
          k++;
        }
      }
      if (group>0){
        print_algorithm(buffer_out_temp[group], node, num_nodes, node_rank, node_row_size, node_column_size, copy_method, counts, counts_max, num_ports, msizes, msizes_max, parameters->data_type, 0, size_level0_l[group], size_level1_l[group], data_l[group], stage_offset, size_level0_l[group-1], size_level1_l[group-1], data_l[group-1], 0, parameters->ascii_out, parameters->root, parameters->bit_identical);
      }else{
        print_algorithm(buffer_out_temp[group], node, num_nodes, node_rank, node_row_size, node_column_size, copy_method, counts, counts_max, num_ports, msizes, msizes_max, parameters->data_type, 0, size_level0_l[group], size_level1_l[group], data_l[group], stage_offset, 0, NULL, NULL, 0, parameters->ascii_out, parameters->root, parameters->bit_identical);
      }
    }else{
      if (group>0){
        print_algorithm(buffer_out_temp[group], node, num_nodes, node_rank, node_row_size, node_column_size, copy_method, counts, counts_max, num_ports, msizes, msizes_max, parameters->data_type, 0, size_level0_l[group], size_level1_l[group], data_l[group], stage_offset, size_level0_l[group-1], size_level1_l[group-1], data_l[group-1], 0, parameters->ascii_out, parameters->root, parameters->bit_identical);
  ...
  free(data_l);
  for (group=0; group<ngroups; group++){
    nbuffer_out+=sprintf(buffer_out+nbuffer_out, "%s", buffer_out_temp[group]);
  }
...
error:
  ...
  free(data_l);
  for (i=ngroups-1; i>=0; i--){
    if (buffer_out_temp) free(buffer_out_temp[i]);
    if (buffer_in_temp) free(buffer_in_temp[i]);
  }
  free(frac_temp);
  free(frac_start);
  free(buffer_out_temp);
