...
static int expand_frac_recursive(struct parameters_block *parameters, int socket, int step, int *fracs) {
  ...
  if (step == 0) {
    for (num_frac = 0; num_frac < abs(group[0]); num_frac++) {
      fracs[num_frac] = (((num_frac + (socket / (parameters->num_nodes / abs(group[0]))) * abs(group[0])) % abs(group[0])) * (parameters->num_nodes / abs(group[0])) + socket) % parameters->num_nodes;
    }
  } else {
    ...
    for (i = 0; i < abs(group[0]); i++) {
      if (step < 0) {
        expand_frac_recursive(parameters, socket_global(parameters, (i + socket_l) % abs(group[0]), step, components), step + 1, fracs + num_frac);
      } else {
        expand_frac_recursive(parameters, socket_global(parameters, (i + socket_l) % abs(group[0]), step, components), step - 1, fracs + num_frac);
      }
      num_frac += num_fracs;
...
static int expand_frac(struct parameters_block *parameters, int socket, int step, int group_middle, int *fracs) {
  int i;
  expand_frac_recursive(parameters, socket, step, fracs);
  for (i = 0; i < parameters->num_nodes / group_middle; i++) {
    fracs[i] = fracs[i * group_middle] % (parameters->num_nodes / group_middle);
  }
  return parameters->num_nodes / group_middle;
}
...
static void revise_frac(int num_nodes, struct data_algorithm *data) {
  int i, j;
  for (j = data->num_blocks - 1; j >= 0; j--) {
    for (i = data->blocks[j].num_lines - 1; i >= 0; i--) {
      data->blocks[j].lines[i].frac = (num_nodes + data->blocks[j].lines[i].frac - data->blocks[0].lines[0].frac) % num_nodes;
    }
  }
}
...
int ext_mpi_generate_allreduce_short(char *buffer_in, char *buffer_out) {
  ...
  if (ngroups[0]+ngroups[1]+ngroups[2] == 1) {
    ...
    return ext_mpi_generate_allreduce_short_single(buffer_in, buffer_out);
  }
  buffer_in_temp = (char *)malloc(1000000);
  buffer_out_temp = (char *)malloc(1000000);
  fracs = (int *)malloc(sizeof(int) * parameters->num_nodes);
  ...
  if (ngroups[1]) {
    ...
    parameters_l->num_nodes = abs(parameters_l->groups[0]);
    parameters_l->node = socket_local(parameters, parameters->node, 0, components);
    ...
    ext_mpi_generate_allreduce_short_single(buffer_in_temp, buffer_out_temp);
    fac_middle = abs(parameters_l->groups[0]);
  ...
  for (i = 0; i < ngroups[0]; i++) {
    ...
    parameters_l->num_nodes = abs(parameters_l->groups[0]);
    parameters_l->node = socket_local(parameters, parameters->node, -i - 1, components);
    nbuffer_in_temp += ext_mpi_write_parameters(parameters_l, buffer_in_temp + nbuffer_in_temp);
    nbuffer_in_temp += ext_mpi_write_eof(buffer_in_temp + nbuffer_in_temp, parameters_l->ascii_out);
    ...
    expand_frac(parameters, parameters->node, -i - 1, fac_middle, fracs);
  ...
  if (!(parameters->message_sizes_max < parameters->num_nodes)) {
