...
int ext_mpi_generate_parallel_memcpy(char *buffer_in, char *buffer_out) {
  ...
  do {
    ...
    read_assembler_line_ssdsdd(line, &estring1, &estring2, &os1, &estring3,
                               &os2, &size, 0);
    estring1_ = estring1;
    estring2_ = estring2;
    estring3_ = estring3;
    ...
    if ((flag3 > 0) && (read_assembler_line_s(line, &estring1, 0) >= 0)) {
      ...
      if (reset) {
        p1 = 0;
        if (flag) {
          p1 += read_line(buffer_in + nbuffer_in + p1, line2,
                          parameters->ascii_in);
        }
        flag = 1;
        size_old = 0;
        while (flag) {
          p1 += flag = read_line(buffer_in + nbuffer_in + p1, line2,
                                 parameters->ascii_in);
          read_assembler_line_ssdsdd(line2, &estring1, &estring2, &os1,
                                     &estring3, &os2, &size, 0);
