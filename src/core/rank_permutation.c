...
void rank_perm_heuristic__(int num_nodes, int *node_recvcounts, int *rank_perm){
  int i, i1, i2, iii;
...
void rank_perm_heuristic_bug(int num_nodes, int *node_recvcounts, int *rank_perm){
  typedef struct node {
    int value, key;
  } node_t;
  node_t array[num_nodes];
  int lnode_recvcounts[num_nodes/2], lrank_perm[num_nodes/2], perm_array[num_nodes];
  int i;
  for (i=0; i<num_nodes; i++){
  ...
  if (num_nodes>1){
    ...
    for (i=0; i<num_nodes/2; i++){
      lnode_recvcounts[i] = array[i].value + array[(num_nodes/2)*2-1-i].value;
    }
...
int generate_rank_permutation_forward(char *buffer_in, char *buffer_out){
  int num_nodes, flag, msizes_max=0, *msizes=NULL, *rank_perm=NULL, *rank_back_perm=NULL;
  ...
  parameters->message_sizes=(int*)malloc(sizeof(int)*parameters->message_sizes_max);
...
int generate_rank_permutation_backward(char *buffer_in, char *buffer_out){
  ...
  int nbuffer_out=0, nbuffer_in=0, i, j, k, size_level0=0, *size_level1=NULL;
