...
int generate_raw_code_tasks_node(char *buffer_in, char *buffer_out){
  int *nodes_recv=NULL, *nodes_send=NULL, node_rank, node_row_size=1, node_column_size=1, node_size;
  int node, num_nodes;
  int nbuffer_out=0, nbuffer_in=0, i, j, k, l, size_level0=0, *size_level1=NULL, size_level0_org=0, *size_level1_org=NULL;
  ...
  if (!rank_back_perm) goto error;
  for (i=0; i<msizes_max; i++){
    rank_back_perm[rank_perm[i]]=i;
  }
  node_size=node_row_size*node_column_size;
  nodes_recv=(int*)malloc(sizeof(int)*num_nodes);
  if (!nodes_recv) goto error;
  nodes_send=(int*)malloc(sizeof(int)*num_nodes);
  if (!nodes_send) goto error;
  i=read_algorithm(buffer_in+nbuffer_in, &size_level0_org, &size_level1_org, &data_org, parameters->ascii_in);
  ...
  for (i=0; i<size_level0; i++){
    ...
    for (j=0; j<size_level1[i]; j++){
      for (k=0; k<data[i][j].from_max; k++){
        if (data[i][j].from_node[k]>=0){
          nodes_recv[(+rank_perm[data[i][j].from_node[k]]-rank_perm[node]+num_nodes)%num_nodes]=(+rank_perm[data[i][j].from_node[k]]-rank_perm[node]+num_nodes)%num_nodes;
        }
        if (data[i][j].from_node[k]<=-10){
          nodes_recv[(+rank_perm[10-data[i][j].from_node[k]]-rank_perm[node]+num_nodes)%num_nodes]=(+rank_perm[10-data[i][j].from_node[k]]-rank_perm[node]+num_nodes)%num_nodes;
        }
      }
      for (k=0; k<data[i][j].to_max; k++){
        if (data[i][j].to[k]>=0){
          nodes_send[(-rank_perm[data[i][j].to[k]]+rank_perm[node]+num_nodes)%num_nodes]=(-rank_perm[data[i][j].to[k]]+rank_perm[node]+num_nodes)%num_nodes;
        }
      }
      for (k=0; k<data[i][j].to_max; k++){
        if (data[i][j].to[k]<=-10){
          nodes_send[(-rank_perm[10-data[i][j].to[k]]+rank_perm[node]+num_nodes)%num_nodes]=(-rank_perm[10-data[i][j].to[k]]+rank_perm[node]+num_nodes)%num_nodes;
        }
      }
    }
    k=0;
    ...
    for (j=0; j<size_level1[i]; j++){
      for (k=0; k<data[i][j].from_max; k++){
        if (data[i][j].from_node[k]>=0){
          l=nodes_recv[(+rank_perm[data[i][j].from_node[k]]-rank_perm[node]+num_nodes)%num_nodes];
          if (l>=0){
            data[i][j].from_node[k]=(+l+rank_perm[node]*node_size+num_nodes*node_size)%(num_nodes*node_size)+node_rank;
            data[i][j].from_node[k]=rank_back_perm[data[i][j].from_node[k]/node_size]*node_size+data[i][j].from_node[k]%node_size;
          }else{
            data[i][j].from_node[k]=-(rank_back_perm[((-l-10+rank_perm[node]*node_size+num_nodes*node_size)%(num_nodes*node_size))/node_size]*node_size)-10-node_rank;
          }
        }
      ...
      for (k=0; k<data[i][j].to_max; k++){
        if (data[i][j].to[k]>=0){
          l=nodes_send[(-rank_perm[data[i][j].to[k]]+rank_perm[node]+num_nodes)%num_nodes];
          if (l>=0){
            data[i][j].to[k]=(-l+rank_perm[node]*node_size+num_nodes*node_size)%(num_nodes*node_size)+node_rank;
            data[i][j].to[k]=rank_back_perm[data[i][j].to[k]/node_size]*node_size+data[i][j].to[k]%node_size;
          }else{
            data[i][j].to[k]=-(rank_back_perm[((+l+10+rank_perm[node]*node_size+num_nodes*node_size)%(num_nodes*node_size))/node_size]*node_size)-10-node_rank;
          }
        }
  ...
  delete_algorithm(size_level0_org, size_level1_org, data_org);
  free(rank_back_perm);
  free(rank_perm);
