...
void ext_mpi_gpu_copy_reduce(char instruction2, void *data, int count) {
  ...
  case OPCODE_REDUCE_SUM_CHAR:
    hipLaunchKernelGGL(gpu_copy_reduce_kernel<char>, (count + 127) / 128, 128, (char *)data);
    break;
  case OPCODE_REDUCE_SUM_DOUBLE:
    hipLaunchKernelGGL(gpu_copy_reduce_kernel<double>, (count + 127) / 128, 128, (char *)data);
    break;
  case OPCODE_REDUCE_SUM_LONG_INT:
    hipLaunchKernelGGL(gpu_copy_reduce_kernel<long int>, (count + 127) / 128, 128, (char *)data);
    break;
  case OPCODE_REDUCE_SUM_FLOAT:
    hipLaunchKernelGGL(gpu_copy_reduce_kernel<float>, (count + 127) / 128, 128, (char *)data);
    break;
  case OPCODE_REDUCE_SUM_INT:
    hipLaunchKernelGGL(gpu_copy_reduce_kernel<int>, count + 127) / 128, 128, (char *)data);
    break;
  }
}
