...
int gpu_sizeof_memhandle(){
  return(sizeof(struct cudaIpcMemHandle_st));
}

int
gpu_setup_shared_memory (MPI_Comm comm, int my_cores_per_node_row,
			 ...
			 char volatile **shmem_gpu)
{
  ...
  if ((my_mpi_rank_row % my_cores_per_node_row == 0)
	...
	exit (16);
      if ((*shmem_gpu) == NULL)
	exit (16);
      if (cudaIpcGetMemHandle ((struct cudaIpcMemHandle_st*)shmemid_gpu, (void *) (*shmem_gpu)) != 0)
	exit (15);
    }
  ...
  if ((*shmem_gpu) == NULL)
    {
	  ...
	  ((void **) shmem_gpu, *((struct cudaIpcMemHandle_st*)shmemid_gpu),
...
int gpu_destroy_shared_memory(MPI_Comm comm, int my_cores_per_node_row, MPI_Comm comm_column, int my_cores_per_node_column, char volatile **shmem_gpu){
  MPI_Comm my_comm_node_h, my_comm_node_v;
  int my_mpi_rank_row, my_mpi_size_row, my_mpi_rank_column,
    my_mpi_size_column;
  MPI_Comm_size (comm, &my_mpi_size_row);
