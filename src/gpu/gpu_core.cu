...
__global__ void gpu_copy_reduce_kernel(char *data){
  ...
  for (i = blockIdx.x*blockDim.x+threadIdx.x; i<num_streams*max_size; i+=blockDim.x*gridDim.x){
    num_stream = i / max_size;
    offset = i % max_size;
    index=0;
    ldata = data + 2*sizeof(int) + sizeof(long int) + (num_streams * index + num_stream) * (sizeof (char *) * 2 + sizeof (long int));
    p1 = *((char**)ldata);
    while (p1){
...
void gpu_copy_reduce(char instruction2, void *data, int count){
  switch (instruction2){
    ...
    case OPCODE_REDUCE_SUM_LONG_INT:
      gpu_copy_reduce_kernel<long int><<<(count+127)/128, 128>>>((char*)data);
      break;
