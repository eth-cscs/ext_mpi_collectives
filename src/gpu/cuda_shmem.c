...
int ext_mpi_sendrecvbuf_init_gpu_blocking(int my_mpi_rank, int my_cores_per_node, int num_sockets, char *sendbuf, char *recvbuf, size_t size, int *mem_partners_send, int *mem_partners_recv, char ***shmem, int **shmem_node, int *counter, char **sendbufs, char **recvbufs) {
  ...
  if (sendbuf != recvbuf) {
    ...
    if (!(((struct address_transfer*)(shmem[0]))[0].present = !insert_address_lookup(&address_lookup_root[mpi_node_rank], sendbuf, size, sendbuf))) {
  ...
  if (!(((struct address_transfer*)(shmem[0]))[1].present = !insert_address_lookup(&address_lookup_root[mpi_node_rank], recvbuf, size, recvbuf))) {
