  ...
  for(num_tasks=mpi_size; num_tasks > 0; num_tasks--){
    if (mpi_rank<num_tasks){
      ...
      while ((size<=MAX_SIZE)&&flag){
        copyin = 0;
        handle = EXT_MPI_Allreduce_init_native(sendbuf, recvbuf, size, MPI_LONG, MPI_SUM, new_comm, num_tasks, MPI_COMM_NULL, 1, num_ports, num_ports_limit, num_active_ports, copyin, 0, 0);
