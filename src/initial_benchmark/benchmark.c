...
int main(int argc, char** argv) {
    ...
    int dest, source, sendcount, recvcount, cores, parallel, iterations, i, j, k, start, step_size, sendcount_array_max, sendcount_array[100000];
...
//        sendcount_array[sendcount_array_max++] = i;
    ...
    setup_shared_memory(MPI_COMM_WORLD, CORES_PER_NODE, MPI_COMM_NULL, 1, sendcount_array[sendcount_array_max-1], &shmemid, &shmem, 0, NUM_BARRIERS);

    sendbuf = (char *)(shmem+NUM_BARRIERS);
//    sendbuf = malloc(sendcount_max);
    recvbuf = (char *)malloc(sendcount_array[sendcount_array_max-1]);
//warn    tempbuf = (char *)malloc(sendcount_array[sendcount_array_max-1]*CORES_PER_NODE);
    parallel = 1;
    for (cores=1; cores<=CORES_PER_NODE&&cores<mpi_size/CORES_PER_NODE; cores++){
        for (k=0; k<sendcount_array_max; k++){
            ...
            for (i=0; i<start; i++){
                if (mpi_rank%CORES_PER_NODE>=cores){
                    ...
                    dest = ((mpi_rank/CORES_PER_NODE+step_size*(mpi_rank%CORES_PER_NODE+1))%(mpi_size/CORES_PER_NODE))*CORES_PER_NODE+mpi_rank%CORES_PER_NODE;
                    source = ((mpi_rank/CORES_PER_NODE+mpi_size-step_size*(mpi_rank%CORES_PER_NODE+1))%(mpi_size/CORES_PER_NODE))*CORES_PER_NODE+mpi_rank%CORES_PER_NODE;
                }
                node_barrier(CORES_PER_NODE);
                if (dest>=0){
                    MPI_Sendrecv(sendbuf, sendcount, MPI_CHAR, dest, 0, recvbuf, recvcount, MPI_CHAR, source, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                }
                node_barrier(CORES_PER_NODE);
                for (j=0; j<1; j++){
//                    memcpy(tempbuf+j*sendcount_max, recvbuf, sendcount_max);
                }
                step_size *= (cores+1);
...
//                    memcpy(tempbuf+j*sendcount_max, recvbuf, sendcount_max);
            ...
            if (mpi_rank==0){
                MPI_Reduce(MPI_IN_PLACE, &wtime_sum, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
//                wtime_sum /= (mpi_size/CORES_PER_NODE*cores*parallel);
                wtime_sum /= iterations;
                printf("%d %d %d %d %e\n", mpi_size, cores, parallel, sendcount, wtime_sum);
            }else{
                MPI_Reduce(&wtime_sum, &wtime_sum, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
            }
