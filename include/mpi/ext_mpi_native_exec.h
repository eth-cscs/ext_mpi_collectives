...
int ext_mpi_exec_blocking(char *ip, MPI_Comm comm, int tag, char **shmem_socket, int *counter_socket, int socket_rank, int num_cores, char **shmem_node, int *counter_node, int num_sockets_per_node, void **shmem_blocking, void **sendbuf, void **recvbuf, int count, int reduction_op, int count_io, void *p_dev_temp);
