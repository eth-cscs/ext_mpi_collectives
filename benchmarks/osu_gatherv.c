    ...
    for(size=options.min_message_size; size <= options.max_message_size; size *= 2) {
...
#ifdef EXT_MPI
        EXT_MPI_Gatherv_init_general (sendbuf, size, MPI_CHAR,
                                  recvbuf, recvcounts, rdispls, MPI_CHAR, 0,
                                  new_comm, NUM_CORES, MPI_COMM_NULL,
                                  1, &handle);
#endif
#ifdef PERSISTENT
        MPIX_Gatherv_init(sendbuf, size, MPI_CHAR, recvbuf, recvcounts, rdispls, MPI_CHAR, 0, new_comm, MPI_INFO_NULL, &req);
#endif
#ifdef EXT_NOPER
        ...
        while (flag){
            ...
            for(i=0; i < options.iterations; i++) {
                t_start = MPI_Wtime();
                MPI_CHECK(MPI_Gatherv(sendbuf, size, MPI_CHAR, recvbuf, recvcounts, rdispls, MPI_CHAR, 0, new_comm));
                t_stop=MPI_Wtime();
...
#else
#ifdef EXT_NOPER
                EXT_MPI_Gatherv (sendbuf, size, MPI_CHAR, recvbuf, recvcounts, rdispls, MPI_CHAR, 0, handle);
#else
                MPI_CHECK(MPI_Gatherv(sendbuf, size, MPI_CHAR, recvbuf, recvcounts, rdispls, MPI_CHAR, 0, new_comm));
#endif
#endif
