#define BENCHMARK "OSU MPI%s Bcast Latency Test"
/*
    ...
    for(size=options.min_message_size; size*sizeof(double) <= options.max_message_size; size *= 2) {
...
#ifdef EXT_MPI
        EXT_MPI_Bcast_init_general (sendbuf, size, MPI_DOUBLE,
                                  0, new_comm, NUM_CORES, MPI_COMM_NULL,
                                  1, &handle);
#endif
#ifdef PERSISTENT
        ...
        while (flag){
            ...
            for(i=0; i < options.iterations; i++) {
                t_start = MPI_Wtime();
                MPI_CHECK(MPI_Bcast(sendbuf, size, MPI_DOUBLE, 0, new_comm));
                t_stop=MPI_Wtime();
...
#else
                MPI_CHECK(MPI_Allreduce(sendbuf, recvbuf, size, MPI_DOUBLE, MPI_SUM, new_comm));
#endif
#endif
#endif
                t_stop=MPI_Wtime();
