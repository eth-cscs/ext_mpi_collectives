...
int main(int argc, char *argv[])
{
    ...
    if (allocate_memory_coll((void**)&displs, numprocs*sizeof(int), NONE)) {
        fprintf(stderr, "Could Not Allocate Memory [rank %d]\n", rank);
        MPI_CHECK(MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE));
    }
    ...
    for(size=options.min_message_size; size*sizeof(double) <= options.max_message_size; size *= 2) {
...
#ifdef EXT_MPI
        EXT_MPI_Scatterv_init_general (sendbuf, recvcounts, displs, MPI_DOUBLE, recvbuf, recvcounts[rank], MPI_DOUBLE,
                                  0,
        ...
        while (flag){
            ...
            for(i=0; i < options.iterations; i++) {
                t_start = MPI_Wtime();
                MPI_CHECK(MPI_Scatterv(sendbuf, recvcounts, displs, MPI_DOUBLE, recvbuf, recvcounts[rank], MPI_DOUBLE, 0, new_comm));
                t_stop=MPI_Wtime();
