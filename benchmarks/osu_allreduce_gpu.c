...
/*    if (allocate_memory_coll((void**)&sendbuf, bufsize, options.accel)) {
    ...
    cudaMalloc((void**)&sendbuf, bufsize);
...
/*    bufsize = sizeof(double)*(options.max_message_size/sizeof(double));
    ...
    cudaMalloc((void**)&recvbuf, bufsize);
...
#ifdef EXT_MPI
        EXT_MPI_Allreduce_init_general (sendbuf, recvbuf, size, MPI_DOUBLE,
                                  MPI_SUM, new_comm, NUM_CORES, MPI_COMM_NULL,
                                  1, &handle);
#endif
#ifdef PERSISTENT
