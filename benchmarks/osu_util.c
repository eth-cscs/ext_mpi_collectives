...
static int set_count_down_size (int value)
{
    if (value < 0 ) {
        return -1;
    }

    options.count_down = value;
...
int process_options (int argc, char *argv[])
{
    ...
    static struct option long_options[] = {
            ...
            {"count-down",      required_argument,  0,  'o'}
    };
    ...
    if(options.bench == PT2PT) {
        ...
        optstring = "+:hvfm:i:x:M:t:a:o:";
        if (accel_enabled) {
            optstring = (CUDA_KERNEL_ENABLED) ? "+:d:hvfm:i:x:M:t:r:a:o:" : "+:d:hvfm:i:x:M:t:a:o:";
        }
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, &option_index)) != -1) {
        ...
        switch(c) {
            ...
            case 's':
                ...
                break;
            case 'o':
                if (set_count_down_size(atoi(optarg))){
                    bad_usage.message = "Invalid Count Down Size";
                    bad_usage.optarg = optarg;
...
void print_stats (int rank, int size, double avg_time, double min_time, double max_time)
{
    ...
    if (options.show_size) {
        fprintf(stdout, "%-*d", 10, size);
        fprintf(stdout, "%*.*f", FIELD_WIDTH, FLOAT_PRECISION, max_time);
    } else {
        fprintf(stdout, "%*.*f", 17, FLOAT_PRECISION, max_time);
    }
